...
function getInputFrequency() {
    return readFile(document.getElementById('AUDIO').files[0]).then(data => {
        ...
        var file = results[0];
        audioContext.decodeAudioData(file.data).then(value => {
...
function run() {
    ...
    var promises = [];
    (toTune.forEach(note => {
        promises.push(tuneToPitch(note.frequency, note.note));
    }));
    notes.forEach(note => {
...
function tuneToPitch(frequency, note) {

    return readFile(document.getElementById('AUDIO').files[0]).then(data => {
        var fileByteArray = new Uint8Array(data);
        var filename = "audio.wav";
        var ratio = frequency / 440;
        var results = ffmpeg_run({
            // -i <input> -af asetrate=44100*0.5,aresample=44100||,atempo=1/2 <output>
