                ...
                .then(() => device.controlTransferOut({
                    ...
                    setInterval(()=>{
                        device.transferIn(5, 64).then(result => {
                            ...
                            var val=parseFloat(decoder.decode(result.data))*parseFloat(document.querySelectorAll("input[type=range]")[0].max);
                            if(val>0){
                                document.querySelectorAll("input[type=range]")[0].value=val;
                            }
                        });
                    },50);
                    */
                }).catch(error => {
                    document.querySelectorAll(".sensor")[0].checked = false;
                });
...
function run() {
    ...
    if (document.querySelectorAll(".sensor")[0].checked) {
        setInterval(() => {
            device.transferIn(5, 64).then(value => {
                let decoder = new TextDecoder();
                sensorValue = parseFloat(decoder.decode(value.data));
            });
        }, 50);
    }
