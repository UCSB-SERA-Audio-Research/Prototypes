    ...
    audioSequence.forEach(event => {
        var playAudio = () => {
            (new Promise((resolve, reject)=>{
                if (device != null) {
                    return resolve(device.transferIn(5, 64));
                } else {
                    return Promise.resolve();
                }
            })).then(result => {
                if (result != null) {
                    var sensorIndex=parseFloat(document.querySelectorAll(".sensorIndex")[0].value);
                    let decoder = new TextDecoder();
                    console.log('Received: ' + decoder.decode(result.data));
                    var val = parseFloat(decoder.decode(result.data)) * parseFloat(document.querySelectorAll("input[type=range]")[sensorIndex].max);
                    if (val > 0) {
                        document.querySelectorAll("input[type=range]")[sensorIndex].value = val;
                    }
                }
